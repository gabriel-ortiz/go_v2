...
class Off_Canvas_Menu_Walker_rendered extends Walker_Nav_Menu {
    function start_lvl(&$output, $depth = 0, $args = Array() ) {
	    ...
	    $output .= "\n$indent<ul class=\"itbl-mobile-dropdown-menu$submenu depth_$depth\">\n";
    }
	...
	function start_el(&$output, $item, $depth=0, $args=array(), $id = 0) {
        ...
        $href_attrs = array(
                ...
                'class'     => ( $item->url == '#' ) ? 'itbl-non-link' : 'itbl-mobile-menu-link',
		...
		$dropdown_wrapper = new html( 'div',
            array(
                    'class' => 'itbl-c-mobile__dropdown-wrapper'
            )
        );
...
add_filter( 'wp_get_nav_menu_items', 'itbl_nav_menu_classes', 10, 3 );

function itbl_nav_menu_classes($items, $menu, $args) {
...
function header_menu( $menu_id, $classname = '' ) {
    ...
    <ul class="itbl-c-menu <?php echo $classname; ?>">
        ...
        <?php foreach( $menu_items as $item ) :
            ...
            <li class="<?php echo implode( ' ', $menu_item_class ); ?>">
                ...
                <?php if( $has_children ): ?>
                    ...
                    <button type="button"  class="js-toggle-header-menu itbl-b-btn" data-toggle="sub-menu-<?php echo $item->ID; ?>">
...
function build_sub_menu_items( $items ) {
    ...
    foreach( $items as $item ) :
        ...
        $has_children = ( !empty( $item->children ) ) ? 'itbl-c-submenu__has-children' : '';

        $item_class = array( "menu-item", "menu-item-type-{$item->type}", "menu-item-object-{$item->object}", "menu-item-{$item->ID}", $has_children, );

        $custom_menu_meta   = get_field( 'itbl_menu_meta', $item->ID );
        $menu_item          = array_search_key( 'itbl_menu__icon', $custom_menu_meta );
        ...
        <li class="<?php echo implode( ' ', $item_class ); ?>">
            ...
            <?php if ( $item->url == '#' ) : ?>
                <div class="itbl-non-link">

                    <?php if( !empty( $menu_item ) ): ?>
                        <img class="itbl-c-non-link__icon" src="<?php echo $menu_item['url'] ?>" aria-hidden="true"/>
                    <?php endif; ?>
                    <div>
                        <div class="itbl-non-link__title h5"><?php echo $item->title; ?></div>
